<script lang="ts">
  import { language } from "$lib/stores/language";
  import { translations } from "$lib/translations";
  import { browser } from "$app/environment";

  export let isOpen = false;
  export let onClose: () => void;
  export let onConfirm: (prNumber: string, baseBranch: string) => void;

  let t = translations.pl;
  let prNumber = "";
  let baseBranch = "";

  $: if (browser) {
    t = translations[$language];
  }

  function handleConfirm() {
    if (prNumber.trim() && baseBranch.trim()) {
      onConfirm(prNumber.trim(), baseBranch.trim());
      onClose();
    }
  }

  function handleKeydown(event: KeyboardEvent) {
    if (event.key === "Escape") {
      onClose();
    }
  }
</script>

{#if isOpen}
  <div
    class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
    on:keydown={handleKeydown}
    role="dialog"
    aria-modal="true"
  >
    <div class="bg-white rounded-lg p-6 w-full max-w-md mx-4">
      <h3 class="text-xl font-bold text-gray-800 mb-4">
        {t.change_specific_base}
      </h3>

      <div class="mb-4">
        <label
          for="pr-number"
          class="block text-sm font-semibold text-gray-700 mb-2"
        >
          {t.pr_number}:
        </label>
        <input
          id="pr-number"
          type="text"
          bind:value={prNumber}
          placeholder="123"
          class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg text-base transition-colors focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100"
        />
      </div>

      <div class="mb-4">
        <label
          for="base-branch"
          class="block text-sm font-semibold text-gray-700 mb-2"
        >
          {t.base_branch}:
        </label>
        <input
          id="base-branch"
          type="text"
          bind:value={baseBranch}
          placeholder="main"
          class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg text-base transition-colors focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100"
        />
      </div>

      <div class="flex gap-3 justify-end">
        <button
          on:click={onClose}
          class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg font-semibold cursor-pointer transition-all duration-300 hover:bg-gray-300"
        >
          {t.cancel}
        </button>
        <button
          on:click={handleConfirm}
          disabled={!prNumber.trim() || !baseBranch.trim()}
          class="px-4 py-2 bg-blue-600 text-white rounded-lg font-semibold cursor-pointer transition-all duration-300 hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed"
        >
          {t.confirm}
        </button>
      </div>
    </div>
  </div>
{/if}
